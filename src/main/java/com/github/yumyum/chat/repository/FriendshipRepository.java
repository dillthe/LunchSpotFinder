...
public interface FriendshipRepository extends JpaRepository<Friendship, Integer> {
    ...
    int exists(@Param("memberId1") int memberId1, @Param("memberId2") int memberId2);

    @Query(value = "SELECT fs FROM friendship fs WHERE fs.member_id1 = :memberId1 AND fs.member_id2 = :memberId2", nativeQuery = true)
